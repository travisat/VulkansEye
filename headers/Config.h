...
static void loadSceneConfig(std::string path, SceneConfig &config) {
  ...
  for (auto &actorconfig : j.at("actors")) {
    ...
    actorconfig.at("model").at("path").get_to(
        config.actors[i].modelConfig.objPath);
    actorconfig.at("model").at("diffuse").get_to(
        config.actors[i].modelConfig.diffusePath);
    actorconfig.at("model").at("normal").get_to(
        config.actors[i].modelConfig.normalPath);
    actorconfig.at("model")
        .at("roughness")
        .get_to(config.actors[i].modelConfig.roughnessPath);
    actorconfig.at("model")
        .at("metallic")
        .get_to(config.actors[i].modelConfig.metallicPath);
    actorconfig.at("model").at("ao").get_to(
        config.actors[i].modelConfig.aoPath);
    actorconfig.at("model")
        .at("displacement")
  ...
  j.at("stage").at("diffuse").get_to(
      config.stageConfig.modelConfig.diffusePath);
  j.at("stage").at("normal").get_to(config.stageConfig.modelConfig.normalPath);
  j.at("stage")
      .at("roughness")
      .get_to(config.stageConfig.modelConfig.roughnessPath);
  j.at("stage")
      .at("metallic")
      .get_to(config.stageConfig.modelConfig.metallicPath);
  ...
  j.at("player")
      .at("timeToReachVMax")
      .get_to(config.playerConfig.timeToReachVMax);
  j.at("player")
      .at("timeToStopFromVMax")
      .get_to(config.playerConfig.timeToStopfromVMax);
  j.at("player").at("jForce").get_to(config.playerConfig.jForce);
  j.at("player")
      .at("mouseSensitivity")
      .get_to(config.playerConfig.mouseSensitivity);
