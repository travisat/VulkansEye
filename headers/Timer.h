...
public:
  ...
  static float getCount() { return getInstance().getCountIMPL(); };

  static std::chrono::steady_clock::time_point getTime() {
    return getInstance().getTimeIMPL();
  };

  static float systemTime() { return getInstance().systemTimeIMPL(); };
...
private:
  ...
  float getCountIMPL() {
    ...
    return std::chrono::duration<float, std::chrono::milliseconds::period>(
