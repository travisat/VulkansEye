...
void VkEngine::createCommandBuffers() {
  ...
  CheckResult(vkAllocateCommandBuffers(vulkan->device, &allocInfo,
                                       commandBuffers.data()));
...
void VkEngine::drawFrame() {
  ...
  CheckResult(vkWaitForFences(vulkan->device, 1,
                              &waitFences[vulkan->currentImage], VK_FALSE,
                              UINT64_MAX));
  CheckResult(
      vkResetFences(vulkan->device, 1, &waitFences[vulkan->currentImage]));
  ...
  VkResult result =
      vkAcquireNextImageKHR(vulkan->device, vulkan->swapChain, UINT64_MAX,
                            presentFinishedSemaphores[vulkan->currentImage],
  ...
  submitInfo.pSignalSemaphores =
      &renderFinishedSemaphores[vulkan->currentImage];
  submitInfo.signalSemaphoreCount = 1;
  submitInfo.pCommandBuffers = &commandBuffers[currentBuffer];
  submitInfo.commandBufferCount = 1;
  CheckResult(vkQueueSubmit(vulkan->graphicsQueue, 1, &submitInfo,
                            waitFences[vulkan->currentImage]));
...
void VkEngine::setupDebugMessenger() {
  ...
  CheckResult(CreateDebugUtilsMessengerEXT(vulkan->instance, &createInfo,
...
QueueFamilyIndices VkEngine::findQueueFamiles(VkPhysicalDevice const &device) {
  ...
  for (const auto &queueFamily : queueFamilies) {
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(device, i, vulkan->surface,
                                         &presentSupport);
...
VkEngine::querySwapChainSupport(VkPhysicalDevice const &device) {
  ...
  vkGetPhysicalDeviceSurfaceFormatsKHR(device, vulkan->surface, &formatCount,
                                       nullptr);
...
void VkEngine::createLogicalDevice() {
  ...
  CheckResult(vkCreateDevice(vulkan->physicalDevice, &createInfo, nullptr,
                             &vulkan->device));

  vkGetDeviceQueue(vulkan->device, indices.graphicsFamily.value(), 0,
                   &vulkan->graphicsQueue);
  vkGetDeviceQueue(vulkan->device, indices.presentFamily.value(), 0,
                   &vulkan->presentQueue);
}
...
void VkEngine::createSwapChain() {
  SwapChainSupportDetails swapChainSupport =
      querySwapChainSupport(vulkan->physicalDevice);
  ...
  CheckResult(vkCreateSwapchainKHR(vulkan->device, &createInfo, nullptr,
                                   &vulkan->swapChain));

  vkGetSwapchainImagesKHR(vulkan->device, vulkan->swapChain, &imageCount,
                          nullptr);
  vulkan->swapChainImages.resize(imageCount);
  vkGetSwapchainImagesKHR(vulkan->device, vulkan->swapChain, &imageCount,
                          vulkan->swapChainImages.data());
...
void VkEngine::createImageViews() {
  ...
  for (size_t i = 0; i < vulkan->swapChainImages.size(); i++) {
    ...
    CheckResult(vkCreateImageView(vulkan->device, &viewInfo, nullptr,
...
void VkEngine::createRenderPass() {
  ...
  CheckResult(vkCreateRenderPass(vulkan->device, &renderPassInfo, nullptr,
                                 &vulkan->renderPass));
}
...
void VkEngine::createFramebuffers() {
  ...
  for (size_t i = 0; i < vulkan->swapChainImageViews.size(); i++) {
    ...
    CheckResult(vkCreateFramebuffer(vulkan->device, &framebufferInfo, nullptr,
...
void VkEngine::createCommandPool() {
  QueueFamilyIndices QueueFamilyIndices =
      findQueueFamiles(vulkan->physicalDevice);

  VkCommandPoolCreateInfo poolInfo = {};
  poolInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
  poolInfo.queueFamilyIndex = QueueFamilyIndices.graphicsFamily.value();

  CheckResult(vkCreateCommandPool(vulkan->device, &poolInfo, nullptr,
                                  &vulkan->commandPool));
}
...
void VkEngine::createColorResources() {
  ...
  colorImage.transitionImageLayout(VK_IMAGE_LAYOUT_UNDEFINED,
...
void VkEngine::createSyncObjects() {
  ...
  for (auto &semaphore : presentFinishedSemaphores) {
    ...
    CheckResult(
        vkCreateSemaphore(vulkan->device, &semaphoreInfo, nullptr, &semaphore));
  }
...
VkSampleCountFlagBits VkEngine::getMaxUsableSampleCount() {
  VkSampleCountFlags counts = std::min(
      vulkan->physicalDeviceProperties.limits.framebufferColorSampleCounts,
