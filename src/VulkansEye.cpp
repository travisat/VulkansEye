...
void VulkansEye::loadConfig(const std::string &path, Config &config)
{
    ...
    else
    {
        ...
        for (auto &[key, lightconfig] : j.at("lights").items())
        {
            ...
            config.pointLights[i].temperature = lightconfig.value("temperature", config.pointLights[i].temperature);
            config.pointLights[i].lumens = lightconfig.value("lumens", config.pointLights[i].lumens);
            ++i;
        }
    }
    ...
    else
    {
        ...
        for (auto &[key, materialConfig] : j.at("materials").items())
        {
            ...
            config.materials[i].diffuse = materialConfig.value("diffuse", config.materials[i].diffuse);
            config.materials[i].normal = materialConfig.value("normal", config.materials[i].normal);
            config.materials[i].metallic = materialConfig.value("metallic", config.materials[i].metallic);
            config.materials[i].roughness = materialConfig.value("roughness", config.materials[i].roughness);
            config.materials[i].ao = materialConfig.value("ao", config.materials[i].ao);
            config.materials[i].displacement = materialConfig.value("displacement", config.materials[i].displacement);
            ++i;
        }
    }
    ...
    else
    {
        ...
        int32_t i = 0;
        for (auto &stagemodel : j.at("models"))
        {
            config.models[i].object = stagemodel.value("object", config.models[i].object);
            config.models[i].material = stagemodel.value("material", config.models[i].material);
