...
void Player::setPerspective(double fieldOfView, double width, double height,
  ...
  this->zFar = zFar;
  perspective =
      glm::perspective(glm::radians(fieldOfView), width / height, zNear, zFar);
  // convert to vulkan
...
void Player::updateAspectRatio(double width, double height) {
  this->windowWidth = width;
  this->windowHeight = height;
  perspective = glm::perspective(glm::radians(fieldOfView), (width / height),
                                 zNear, zFar);
  // convert to vulkan
  perspective = clip * perspective;
}
...
void Player::update(float deltaTime) {
  ...
  if (Input::checkKeyboard(GLFW_KEY_W)) {
    direction += camFront *
                 glm::vec3(1.0f, 0.0f, 1.0f); // forward with way player looking
  }
  ...
  if (direction != glm::vec3(0.0f)) {
    ...
    if (glm::abs(velocity.x) < 0.001f &&
        glm::abs(velocity.z) < 0.001) // if stopped don't apply friction
    {
      velocity.x = 0.0f;
      velocity.z = 0.0f;
    } else // apply friction
    {
      float internalFriction =
          (glm::length(velocity) / velocityMax) *
          walkingForce; // internal friction, our legs only move so fast
      force -= internalFriction * normalize(velocity);
    }
  } else if (position.y == -height) { // we want to stop
    ...
    {
      float internalFriction =
          (glm::length(velocity) / velocityMax) * stoppingForce;
