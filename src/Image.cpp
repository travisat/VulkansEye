...
void Image::loadSTB(std::string path) {
  ...
  stbi_uc *pixels =
      stbi_load(path.c_str(), &width, &height, &channels, defaultChannels);
...
void Image::allocate() {
  ...
  CheckResult(vmaCreateImage(vulkan->allocator, &imageInfo, &allocInfo, &image,
                             &allocation, nullptr));
}
...
void Image::createImageView(VkImageViewType viewType,
  ...
  CheckResult(
      vkCreateImageView(vulkan->device, &viewInfo, nullptr, &imageView));
}
...
void Image::copyFrom(const Buffer &buffer, uint32_t layerCount) {
  ...
  vkCmdCopyBufferToImage(commandBuffer, buffer.buffer, image,
...
void Image::generateMipmaps() {
  ...
  vkGetPhysicalDeviceFormatProperties(vulkan->physicalDevice, format,
                                      &formatProperties);
...
void Image::transitionImageLayout(VkImageLayout oldLayout,
  ...
  if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED &&
    ...
    Trace("Unsupported Transition for ", name, " using default values at ",
          Timer::systemTime());
  }
