...
void Model::loadImage(const std::string &path, Image &image,
                      VkSampler &sampler) {
  image.vulkan = vulkan;
  image.imageUsage = VK_IMAGE_USAGE_TRANSFER_SRC_BIT |
  ...
  CheckResult(
      vkCreateSampler(vulkan->device, &imageSamplerInfo, nullptr, &sampler));
}
