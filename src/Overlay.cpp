...
void Overlay::createFont() {
  ...
  vkCmdCopyBufferToImage(copyCmd, stagingBuffer.buffer, fontImage.image,
...
void Overlay::createPipelineLayout() {
  ...
  CheckResult(vkCreatePipelineLayout(vulkan->device, &pipelineLayoutCreateInfo,
...
void Overlay::newFrame() {
  ...
  if (((frameTime - lastUpdateTime) > updateFreqTime) ||
      (lastUpdateTime == 0.0f)) {
    lastUpdateTime = frameTime;
    uiSettings.position =
        player->position * -1.0f; // world is opposite cameras position
...
void Overlay::updateBuffers() {
  ...
  if (imDrawData) {
    ...
    update = (vertexBuffer.buffer == VK_NULL_HANDLE) ||
