...
vec3 convertSRGBtoLinear(vec3 srgbIn) {
  ...
  vec3 linOut =
      mix(srgbIn / 12.92, pow((srgbIn + 0.055) / 1.055, vec3(2.4)), bLess);
  return linOut;
}
...
float SmithGGXCorrelated(float NdotL, float NdotV, float roughness) {
  ...
  //  is not a mistake.
  float Lambda_GGXV =
      NdotL * sqrt((-NdotV * alphaSq + NdotV) * NdotV + alphaSq);
  float Lambda_GGXL =
      NdotV * sqrt((-NdotL * alphaSq + NdotL) * NdotL + alphaSq);
...
vec3 BRDF(vec3 N, vec3 V, vec3 L, vec3 baseColor, float roughness,
  ...
  float NdotH =
      clamp(dot(N, H), 0.0, 1.0); // cos angle between normal and half vector
...
vec3 kelvinToRGB(float kelvin) {
  ...
  if (kelvin <= 66.0) {
    color.r = 1.0;
    color.g = clamp(-0.606464 - 0.001742 * (kelvin - 2.0) +
                        0.408173 * log(kelvin - 2.0),
                    0.0, 1.0);
    if (kelvin <= 20.0)
      color.b = 0.0;
    else
      color.b = clamp(-0.995193 + 0.000323 * (kelvin - 10.0) +
                          0.451875 * log(kelvin - 10.0),
                      0.0, 1.0);
  } else {
    color.r = clamp(1.374910 - 0.001742 * (kelvin - 55.0) +
                        0.408173 * log(kelvin - 55.0),
                    0.0, 1.0);
    color.g = clamp(1.271287 - 0.00031 * (kelvin - 50.0) -
                        0.109708 * log(kelvin - 50.0),
                    0.0, 1.0);
    color.b = 1.0;
  }
  return color;
...
void main() {
  ...
  for (int i = 0; i < numLights; ++i) {
    ...
    float intensity =
        lumens * getDistanceAttenuation(lightVector, lumens) / (4 * PI);
