...
void main()
{
	...
	outUV = gl_TessCoord[0] * inUV[0]
          + gl_TessCoord[1] * inUV[1]
          + gl_TessCoord[2] * inUV[2];
	outNormal = gl_TessCoord[0] * inNormal[0]
	...
	// build terms
	vec3 termIJ = vec3(inPhongPatch[0].termIJ,
                       inPhongPatch[1].termIJ,
                   	   inPhongPatch[2].termIJ);
	vec3 termJK = vec3(inPhongPatch[0].termJK,
                   	   inPhongPatch[1].termJK,
                       inPhongPatch[2].termJK);
	vec3 termIK = vec3(inPhongPatch[0].termIK,
                       inPhongPatch[1].termIK,
